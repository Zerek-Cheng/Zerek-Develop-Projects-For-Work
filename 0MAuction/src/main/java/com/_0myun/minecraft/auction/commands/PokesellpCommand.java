...
public class PokesellpCommand implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        Player p = (Player) sender;if (!p.hasPermission("auction.points"))return true;
        if (args.length != 5) {
        ...
        AuctionPreSellEvent preEvent = new AuctionPreSellEvent(p
                , PaywayManager.getPayway().get("points")
