...
public abstract class AbstractCommandHelper extends CommandHelper
{
    ...
    public void ctor() {
        ...
        for (final Class c : this.getClass().getAnnotation(Command.class).sub()) {
            try {
                final String name = c.getClass().getAnnotation(Command.class).name().toUpperCase();
    ...
    public Boolean processCommand(final ICommandSender name, String[] args) {
        this.pcParam = name;
        ...
        if ((cmd.equals("HELP") || args.length == 0) && this.doHelp(name, args, cmd)) {
            return true;
        }
        ...
        if (ch instanceof AbstractCommandHelper) {
            ...
            return f.processCommand(name, args);
        }
    ...
    private boolean doHelp(final ICommandSender name, final String[] args, String cmd) {
        ...
        if (ch != null) {
            ...
            if (ch instanceof AbstractCommandHelper) {
                ((AbstractCommandHelper)ch).pcParam = name;
    ...
    protected class MethodSubCmd extends CommandHelper
    {
        ...
        public List addTabCompletion(final ICommandSender par1, final String[] args) {
            ...
            final String nameeter = np[args.length - 1];
            if (nameeter.equals("<player>")) {
