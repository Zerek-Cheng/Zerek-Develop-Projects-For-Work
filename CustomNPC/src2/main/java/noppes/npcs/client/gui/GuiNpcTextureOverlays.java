...
public class GuiNpcTextureOverlays extends GuiNpcSelectionInterface
{
    ...
    public void initGui() {
        super.initGui();
        final int index = this.npc.display.glowTexture.lastIndexOf("/");
        if (index > 0) {
            final String asset = this.npc.display.glowTexture.substring(index + 1);
            if (this.npc.display.glowTexture.equals(this.assets.getAsset(asset))) {
    ...
    public void elementClicked() {
        if (this.dataTextures.contains(this.slot.selected) && this.slot.selected != null) {
            this.npc.display.glowTexture = this.assets.getAsset(this.slot.selected);
