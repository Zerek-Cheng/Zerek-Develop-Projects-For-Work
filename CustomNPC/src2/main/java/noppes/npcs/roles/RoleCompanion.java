...
public class RoleCompanion extends RoleInterface
{
    ...
    public boolean canWearArmor(final ItemStack item) {
        ...
        final int reduction = (int)ObfuscationReflectionHelper.getPrivateValue((Class)ItemArmor.ArmorMaterial.class, (Object)armor.getArmorMaterial(), 5);
    ...
    private ItemStack getFood() {
        ...
        while (ite.hasNext()) {
            ...
            if (is == null || !(is.getItem() instanceof ItemFood)) {
                ...
                final int amount = ((ItemFood)is.getItem()).getHealAmount(is);
                if (i != -1 && amount >= i) {
                    continue;
                }
                i = amount;
        ...
        for (final ItemStack is2 : food) {
            if (((ItemFood)is2.getItem()).getHealAmount(is2) == i) {
                return is2;
            }
        }
        return null;
