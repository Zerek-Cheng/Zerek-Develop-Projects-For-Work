...
public class ResidencePlayerListener
    ...
    private static int[] $SWITCH_TABLE$com$bekvon$bukkit$residence$protection$FlagPermissions$FlagState;
    private static int[] $SWITCH_TABLE$cmiLib$ItemManager$CMIMaterial;
    ...
    public void onCommand(PlayerCommandPreprocessEvent event) {
        ...
        for (String oneBlack : res.getCmdBlackList()) {
            ...
            if (!msg.startsWith("/" + t)) continue;
            black = msg.contains("_") ? t.split("_").length : 1;
            break;
        }
        if (black == 0) {
            for (String oneBlack : res.getCmdBlackList()) {
    ...
    public void onPlayerWorldChange(PlayerChangedWorldEvent event) {
        ...
        if (!(!player.getAllowFlight() && !player.isFlying() || !perms.has(Flags.nofly, false) || this.plugin.isResAdminOn(player) || player.hasPermission("residence.nofly.bypass") || (res = this.plugin.getResidenceManager().getByLoc(player.getLocation())) != null && res.isOwner(player))) {
            ...
            for (int i = 0; i < maxH; ++i) {
                ...
                if (location.getBlockY() > 0) continue;
    ...
    private boolean isCanUseEntity_RClickOnly(Material mat, Block block) {
        ...
        switch (string.hashCode()) {
            case -2063836287: {
                if (!string.equals("REDSTONE_COMPARATOR")) break;
                return true;
            }
            case -1892221527: {
                if (!string.equals("DIODE_BLOCK_OFF")) break;
                return true;
            }
            case -1629786261: {
                if (!string.equals("DAYLIGHT_DETECTOR")) break;
                return true;
            }
            case -1331883119: {
                if (!string.equals("REDSTONE_COMPARATOR_OFF")) break;
                return true;
            }
            case -1289889955: {
                if (!string.equals("REDSTONE_COMPARATOR_ON")) break;
                return true;
            }
            case -753776059: {
                if (!string.equals("DIODE_BLOCK_ON")) break;
                return true;
            }
            case -562844383: {
                if (!string.equals("ITEM_FRAME")) break;
                return true;
            }
            case -415523425: {
                if (!string.equals("WORKBENCH")) break;
                return true;
            }
            case -343861338: {
                if (!string.equals("CAKE_BLOCK")) break;
                return true;
            }
            case -329276657: {
                if (!string.equals("BED_BLOCK")) break;
                return true;
            }
            case 62437548: {
                if (!string.equals("ANVIL")) break;
                return true;
            }
            case 65052267: {
                if (!string.equals("DIODE")) break;
                return true;
            }
            case 867723593: {
                if (!string.equals("DAYLIGHT_DETECTOR_INVERTED")) break;
                return true;
            }
            case 1401892433: {
                if (!string.equals("FLOWER_POT")) break;
                return true;
            }
            case 1545025079: {
                if (!string.equals("BREWING_STAND")) break;
                return true;
            }
            case 1593989766: {
                if (!string.equals("ENCHANTMENT_TABLE")) break;
                return true;
            }
            case 1668377387: {
...
*/
    ...
    private static Location getSafeLocation(Location loc) {
        ...
        for (i = 0; i <= loc.getWorld().getMaxHeight() - curY; ++i) {
            block = loc.clone().add(0.0, (double) i, 0.0).getBlock();
            if (block.isEmpty() || !block.getLocation().clone().add(0.0, 1.0, 0.0).getBlock().isEmpty() || !block.getLocation().clone().add(0.0, 2.0, 0.0).getBlock().isEmpty())
                continue;
            return loc.clone().add(0.0, (double) (i + 1), 0.0);
    ...
    public void onResidenceChangeMessagePrint(ResidenceChangedEvent event) {
        ...
        ClaimedResidence claimedResidence = res;
    ...
    static int[] $SWITCH_TABLE$com$bekvon$bukkit$residence$protection$FlagPermissions$FlagState() {
        int[] arrn2 = $SWITCH_TABLE$com$bekvon$bukkit$residence$protection$FlagPermissions$FlagState;
        if (arrn2 != null) {
            return arrn2;
        }
        int[] arrn = new int[FlagPermissions.FlagState.values().length];
        try {
    ...
    static int[] $SWITCH_TABLE$cmiLib$ItemManager$CMIMaterial() {
        int[] arrn2 = $SWITCH_TABLE$cmiLib$ItemManager$CMIMaterial;
        if (arrn2 != null) {
            return arrn2;
        }
        int[] arrn = new int[ItemManager.CMIMaterial.values().length];
        try {
