...
public class Loader implements Runnable {
    ...
    private Logger getLogger() {
        return VexRedEnvelope.plugin.getLogger();
    }
    ...
    public void deal(int msgType, byte[] msg) {
        switch (msgType) {
            case 1001:
                ...
                licenseCheckPackage.setLicenseType("VexRedEnvelope");
                licenseCheckPackage.setPublicKey(this.getPublicKey());
                licenseCheckPackage.setVersion(VexRedEnvelope.plugin.getDescription().getVersion());
            ...
            case 2004:
                ...
                try {
                    new ClassLoader(VexRedEnvelope.plugin.getClass().getClassLoader()) {
    ...
    public void run() {
        ...
        try {
            license = 100;
            licenseString = VexRedEnvelope.plugin.getConfig().getString("license");
            getLogger().log(Level.INFO, "VexRedEnvelope Loading");
            try {
                //主要验证步骤
            ...
            Bukkit.getPluginManager().disablePlugin(VexRedEnvelope.plugin);
            e.printStackTrace();
        } finally {
            if (license != 1) {
                try {
